...
* rest[0].resource.interaction[0].documentation = "This interaction corresponds to the IHE PCC-X1 and PCC-X2 transaction."
...
For the PCC-X1 transaction, the server need only to respond to search requests where
_summary=true to restrict access to the full questionnaire content. This search parameter is not used in
the PCC-X2 transaction.
"""
...
and is used with the PCC-X1 transaction.
...
their status (i.e., draft | active | retired | unknown) and is used with the PCC-X1 transaction.
It must be documented in the capability statement
"""
...
* rest[0].resource.searchParam[5].documentation = """
This search parameter allows assessment instruments to be located based upon
their canonical uri and is used with the PCC-X2 transaction.
It must be documented in the capability statement
"""